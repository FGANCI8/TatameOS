...
export default function FeedbacksDoAlunoPage() {
  ...
  useEffect(() => {
    if (userId) {
      void listarFeedbacksPorAluno(userId);
    }
    ...
    <main className="relative mx-auto max-w-6xl space-y-8 px-4 pb-28 pt-20 md:px-6">
        ...
        <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            <Link
              ...
              className="inline-flex items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-950/60 px-6 py-3 text-xs font-black uppercase tracking-widest text-white transition-colors hover:bg-zinc-800"
            >
              Voltar ao dashboard
            </Link>
            <button
              onClick={openModal}
              className="inline-flex items-center justify-center rounded-2xl border border-brand-red/20 bg-brand-red/10 px-6 py-3 text-xs font-black uppercase tracking-widest text-brand-red transition-colors hover:bg-brand-red/20 active:scale-95"
            >
              Novo feedback
            </button>
          </div>
            ...
            <div className="flex flex-col gap-6 lg:flex-row lg:items-start">
              <div className="flex-1">
                <div className="mb-4 flex flex-wrap items-center gap-2">
                  <span className={`rounded-full px-3 py-1 text-[10px] uppercase tracking-[0.2em] font-black ${statusClasses[feedback.status]}`}>
                ...
                <h2 className="text-2xl font-black uppercase tracking-tight text-white">{feedback.titulo}</h2>
      ...
      {modalAberto ? (
        <div className="fixed inset-0 z-[200] flex items-end md:items-center md:justify-center">
          ...
          <aside className="relative max-h-[90vh] w-full overflow-y-auto rounded-t-[28px] border border-zinc-800 bg-zinc-900 md:rounded-[28px] md:max-w-3xl">
            <div className="flex items-start justify-between gap-4 border-b border-zinc-800 p-6 md:p-8">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-black uppercase tracking-tighter text-white md:text-4xl">
                  Relatar falha
                </h2>
              </div>
              <button
                type="button"
                onClick={closeModal}
                className="flex h-11 w-11 items-center justify-center rounded-2xl border border-zinc-800 bg-zinc-950/60 text-white transition-colors hover:bg-zinc-800"
              >
            ...
            <form onSubmit={handleSubmit} className="space-y-5 p-6 md:p-8">
              ...
              <div className="flex flex-col gap-3 border-t border-zinc-800 pt-5 sm:flex-row sm:justify-end">
                ...
                <button
                  type="submit"
                  disabled={enviando}
                  className="inline-flex items-center justify-center rounded-2xl border border-brand-red/20 bg-brand-red/10 px-6 py-3 text-xs font-black uppercase tracking-widest text-brand-red transition-colors hover:bg-brand-red/20 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60"
                >
