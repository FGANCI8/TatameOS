...
export function NotificationCenterWidget({
  ...
  const { notificacoes, loading, error, actions } = useNotifications(limite, selectedGym?.id ?? null);
  ...
  if (!selectedGym) {
    return (
      <section className="rounded-[28px] border border-zinc-800/80 bg-zinc-900/80 p-5 shadow-none md:p-6">
        ...
        <div className="mt-5 rounded-2xl border border-dashed border-zinc-800 bg-zinc-950/60 p-6 text-sm text-zinc-400">
          Selecione uma academia para ver os avisos
        </div>
      </section>
    );
  }
