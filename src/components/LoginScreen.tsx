...
export function LoginScreen() {
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-zinc-950 px-4 py-6 font-body text-white sm:px-6 sm:py-8">
      ...
      <div className="relative z-10 w-full max-w-[440px] rounded-[28px] border border-zinc-800 bg-zinc-900/85 p-6 shadow-[0_20px_60px_rgba(0,0,0,0.45)] backdrop-blur-xl sm:p-8 md:p-10">
        <div className="mb-8 flex flex-col items-center text-center">
          ...
          <h1 className="mt-4 font-headline text-4xl font-black uppercase tracking-[0.2em] text-white">
            Tatame <span className="text-brand-red">OSS</span>
          </h1>
          <p className="mt-3 max-w-[28ch] text-sm leading-6 text-zinc-400">
            Entre para acessar sua academia. Se a conta não existir ou a senha estiver errada, o sistema mostra o motivo sem mascarar o erro.
          </p>
        </div>

        <p className="mb-8 border-y border-zinc-800 py-2 text-center text-[10px] font-black uppercase tracking-[0.4em] text-zinc-400">
          {isLoginMode ? 'Acessar conta' : 'Criar conta'}
        </p>

        {error ? (
          <div
            className="mb-6 rounded-2xl border border-brand-red/30 bg-brand-red/10 p-4 text-center text-xs font-bold uppercase tracking-wider text-brand-red"
            role="alert"
            aria-live="assertive"
          >
            {error}
        ...
        {resetMessage ? (
          <div
            className="mb-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/10 p-4 text-center text-xs font-bold uppercase tracking-wider text-emerald-300"
            role="status"
            aria-live="polite"
          >
            {resetMessage}
