import { Buffer } from 'node:buffer';
import { randomUUID } from 'node:crypto';
import { Timestamp } from 'firebase-admin/firestore';
import { adminDb, adminStorage } from '../../../lib/firebase-admin';
import type { ImageGenerationProvider, ImageGenerationRequest, ImageGenerationResult } from '../contracts/image-generation.contract';

type ManifestStatus = 'generating' | 'generated' | 'failed';

interface VisualImageManifestDocument {
  hashKey: string;
  tenantId?: string;
  screenId: string;
  module: string;
  stateType?: string;
  imagePrompt: string;
  imageConcept: string;
  status: ManifestStatus;
  providerUsed?: ImageGenerationProvider | string;
  imageUrl?: string;
  storagePath?: string;
  errorMessage?: string;
  createdAt: Timestamp;
  updatedAt: Timestamp;
  generatedAt?: Timestamp;
}

interface VisualImageReservationResult {
  cachedResult?: ImageGenerationResult;
  alreadyGenerating?: boolean;
}

interface VisualImageStoreConfig {
  cacheCollection?: string;
  usageCollection?: string;
  bucketName?: string;
  dailyLimitPerTenant?: number;
  generationLockTtlMs?: number;
}

const DEFAULT_CACHE_COLLECTION = 'visual_image_cache';
const DEFAULT_USAGE_COLLECTION = 'visual_image_usage';
const DEFAULT_DAILY_LIMIT = 20;
const DEFAULT_GENERATION_LOCK_TTL_MS = 5 * 60 * 1000;
const STORAGE_CONTENT_TYPE = 'image/png';

function readEnvNumber(name: string, fallback: number): number {
  const raw = process.env[name]?.trim();
  if (!raw) {
    return fallback;
  }

  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed <= 0) {
    return fallback;
  }

  return Math.floor(parsed);
}

function manifestToResult(manifest: VisualImageManifestDocument): ImageGenerationResult {
  return {
    imageUrl: manifest.imageUrl,
    status: manifest.status === 'generated' ? 'cached' : 'fallback',
    providerUsed: (manifest.providerUsed as ImageGenerationProvider) ?? 'fallback',
    generatedAt: manifest.generatedAt?.toDate().toISOString() ?? manifest.updatedAt.toDate().toISOString(),
    hashKey: manifest.hashKey,
  };
}

function decodeDataUrl(dataUrl: string): { buffer: Buffer; contentType: string } {
  const match = dataUrl.match(/^data:([^;]+);base64,(.+)$/);
  if (!match) {
    throw new Error('URL de dados inválida.');
  }

  const [, contentType, base64] = match;
  return {
    buffer: Buffer.from(base64, 'base64'),
    contentType,
  };
}

function buildDownloadToken(): string {
  return randomUUID().replace(/-/g, '');
}

function buildStoragePublicUrl(bucketName: string, storagePath: string, token: string): string {
  return `https://firebasestorage.googleapis.com/v0/b/${encodeURIComponent(bucketName)}/o/${encodeURIComponent(storagePath)}?alt=media&token=${encodeURIComponent(token)}`;
}

function buildUsageDocId(tenantId: string, dayKey: string): string {
  return `${tenantId}__${dayKey}`;
}

function buildDayKey(date = new Date()): string {
  return new Intl.DateTimeFormat('en-CA', {
    timeZone: 'America/Sao_Paulo',
  }).format(date);
}

function isWithinTtl(updatedAt: Timestamp, ttlMs: number): boolean {
  return Date.now() - updatedAt.toMillis() < ttlMs;
}

function isGeneratedManifest(manifest?: VisualImageManifestDocument | null): manifest is VisualImageManifestDocument {
  return Boolean(manifest && manifest.status === 'generated' && manifest.imageUrl);
}

function isFreshGeneratingManifest(manifest?: VisualImageManifestDocument | null, ttlMs = DEFAULT_GENERATION_LOCK_TTL_MS): boolean {
  return Boolean(manifest && manifest.status === 'generating' && isWithinTtl(manifest.updatedAt, ttlMs));
}

function isFreshFailedManifest(manifest?: VisualImageManifestDocument | null, ttlMs = DEFAULT_GENERATION_LOCK_TTL_MS): boolean {
  return Boolean(manifest && manifest.status === 'failed' && isWithinTtl(manifest.updatedAt, ttlMs));
}

function parseImageBytes(imageUrl: string): Promise<{ buffer: Buffer; contentType: string }> | { buffer: Buffer; contentType: string } {
  if (imageUrl.startsWith('data:')) {
    return decodeDataUrl(imageUrl);
  }

  return fetch(imageUrl).then(async (response) => {
    if (!response.ok) {
      throw new Error(`Falha ao ler imagem gerada: ${response.status} ${response.statusText}`);
    }

    const contentType = response.headers.get('content-type')?.trim() || STORAGE_CONTENT_TYPE;
    return {
      buffer: Buffer.from(await response.arrayBuffer()),
      contentType,
    };
  });
}

export class VisualImageStore {
  private readonly cacheCollection: string;
  private readonly usageCollection: string;
  private readonly bucketName?: string;
  private readonly dailyLimitPerTenant: number;
  private readonly generationLockTtlMs: number;

  constructor(config: VisualImageStoreConfig = {}) {
    this.cacheCollection = config.cacheCollection ?? DEFAULT_CACHE_COLLECTION;
    this.usageCollection = config.usageCollection ?? DEFAULT_USAGE_COLLECTION;
    this.bucketName = config.bucketName ?? process.env.FIREBASE_STORAGE_BUCKET?.trim();
    this.dailyLimitPerTenant = config.dailyLimitPerTenant ?? readEnvNumber('VISUAL_IMAGE_MAX_GENERATIONS_PER_TENANT_PER_DAY', DEFAULT_DAILY_LIMIT);
    this.generationLockTtlMs = config.generationLockTtlMs ?? DEFAULT_GENERATION_LOCK_TTL_MS;
  }

  hasPersistentStorage(): boolean {
    return Boolean(this.bucketName);
  }

  private manifestRef(hashKey: string) {
    return adminDb.collection(this.cacheCollection).doc(hashKey);
  }

  private usageRef(tenantId: string, dayKey: string) {
    return adminDb.collection(this.usageCollection).doc(buildUsageDocId(tenantId, dayKey));
  }

  private bucket() {
    if (!this.bucketName) {
      return null;
    }

    return adminStorage.bucket(this.bucketName);
  }

  async getManifest(hashKey: string): Promise<VisualImageManifestDocument | null> {
    const snap = await this.manifestRef(hashKey).get();
    if (!snap.exists) {
      return null;
    }

    return snap.data() as VisualImageManifestDocument;
  }

  async getCachedResult(hashKey: string): Promise<ImageGenerationResult | null> {
    const manifest = await this.getManifest(hashKey);
    if (!isGeneratedManifest(manifest)) {
      return null;
    }

    return manifestToResult(manifest);
  }

  async waitForGeneratedResult(hashKey: string, maxAttempts = 6, delayMs = 200): Promise<ImageGenerationResult | null> {
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
      const cached = await this.getCachedResult(hashKey);
      if (cached) {
        return cached;
      }

      await new Promise((resolve) => setTimeout(resolve, delayMs));
    }

    return null;
  }

  async reserveGenerationSlot(request: ImageGenerationRequest, hashKey: string): Promise<VisualImageReservationResult> {
    const manifestRef = this.manifestRef(hashKey);
    const dayKey = buildDayKey();
    const usageRef = this.usageRef(request.tenantId ?? 'global', dayKey);
    const now = Timestamp.now();

    const result = await adminDb.runTransaction(async (transaction) => {
      const manifestSnap = await transaction.get(manifestRef);
      const manifest = manifestSnap.exists ? (manifestSnap.data() as VisualImageManifestDocument) : null;

      if (isGeneratedManifest(manifest)) {
        return { cachedResult: manifestToResult(manifest) } satisfies VisualImageReservationResult;
      }

      if (isFreshGeneratingManifest(manifest, this.generationLockTtlMs)) {
        return { alreadyGenerating: true } satisfies VisualImageReservationResult;
      }

      if (isFreshFailedManifest(manifest, this.generationLockTtlMs)) {
        return { alreadyGenerating: false } satisfies VisualImageReservationResult;
      }

      const usageSnap = await transaction.get(usageRef);
      const currentCount = Number(usageSnap.exists ? usageSnap.data()?.count ?? 0 : 0);
      if (currentCount >= this.dailyLimitPerTenant) {
        throw new Error('VISUAL_IMAGE_TENANT_LIMIT_REACHED');
      }

      transaction.set(usageRef, {
        tenantId: request.tenantId ?? 'global',
        dayKey,
        count: currentCount + 1,
        limit: this.dailyLimitPerTenant,
        updatedAt: now,
        createdAt: usageSnap.exists ? usageSnap.data()?.createdAt ?? now : now,
      });

      transaction.set(manifestRef, {
        hashKey,
        tenantId: request.tenantId,
        screenId: request.screenId,
        module: request.module,
        stateType: request.stateType,
        imagePrompt: request.imagePrompt,
        imageConcept: request.imageConcept,
        status: 'generating',
        createdAt: manifestSnap.exists ? (manifestSnap.data() as VisualImageManifestDocument).createdAt : now,
        updatedAt: now,
      });

      return { alreadyGenerating: false } satisfies VisualImageReservationResult;
    });

    return result;
  }

  async persistGeneratedImage(
    request: ImageGenerationRequest,
    hashKey: string,
    generated: ImageGenerationResult,
  ): Promise<ImageGenerationResult> {
    const bucket = this.bucket();
    const manifestRef = this.manifestRef(hashKey);
    const now = Timestamp.now();

    if (!generated.imageUrl) {
      await manifestRef.set(
        {
          hashKey,
          tenantId: request.tenantId,
          screenId: request.screenId,
          module: request.module,
          stateType: request.stateType,
          imagePrompt: request.imagePrompt,
          imageConcept: request.imageConcept,
          status: 'failed',
          providerUsed: generated.providerUsed,
          errorMessage: generated.imageUrl ? undefined : 'Imagem gerada sem URL persistível.',
          updatedAt: now,
          generatedAt: now,
        },
        { merge: true },
      );

      return {
        ...generated,
        status: 'fallback',
        generatedAt: now.toDate().toISOString(),
        hashKey,
      };
    }

    const storagePath = `visual-experience/${request.tenantId ?? 'global'}/${hashKey}.png`;
    let imageUrl = generated.imageUrl;

    if (bucket) {
      const imageData = await parseImageBytes(generated.imageUrl);
      const downloadToken = buildDownloadToken();
      const file = bucket.file(storagePath);

      await file.save(imageData.buffer, {
        resumable: false,
        metadata: {
          contentType: imageData.contentType || STORAGE_CONTENT_TYPE,
          metadata: {
            firebaseStorageDownloadTokens: downloadToken,
            hashKey,
            tenantId: request.tenantId ?? 'global',
            screenId: request.screenId,
            module: request.module,
          },
        },
      });

      imageUrl = buildStoragePublicUrl(bucket.name, storagePath, downloadToken);
    }

    await manifestRef.set(
      {
        hashKey,
        tenantId: request.tenantId,
        screenId: request.screenId,
        module: request.module,
        stateType: request.stateType,
        imagePrompt: request.imagePrompt,
        imageConcept: request.imageConcept,
        status: 'generated',
        providerUsed: generated.providerUsed,
        imageUrl,
        storagePath: bucket ? storagePath : undefined,
        updatedAt: now,
        generatedAt: now,
      },
      { merge: true },
    );

    return {
      ...generated,
      imageUrl,
      status: 'generated',
      generatedAt: now.toDate().toISOString(),
      hashKey,
    };
  }

  async markFailed(request: ImageGenerationRequest, hashKey: string, reason: string, providerUsed?: ImageGenerationProvider): Promise<void> {
    await this.manifestRef(hashKey).set(
      {
        hashKey,
        tenantId: request.tenantId,
        screenId: request.screenId,
        module: request.module,
        stateType: request.stateType,
        imagePrompt: request.imagePrompt,
        imageConcept: request.imageConcept,
        status: 'failed',
        providerUsed,
        errorMessage: reason,
        updatedAt: Timestamp.now(),
        generatedAt: Timestamp.now(),
      },
      { merge: true },
    );
  }
}
